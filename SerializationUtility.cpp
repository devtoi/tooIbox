...
size_t SerializationUtility::GetPointerDiff( const void* lhs, const void* rhs ) {
...
void SerializationUtility::WritePointer( void* value, Byte*& destination ) {
	CopyAndIncrementDestination( destination, &value, POINTER_SIZE );
}
...
void SerializationUtility::ReadPointer( void*& value, const Byte*& source ) {
	CopyAndIncrementSource( &value, source, POINTER_SIZE );
}
