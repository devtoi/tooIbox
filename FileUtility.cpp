...
//  TODODB: Work trough this file and make naming, naming and methods consistent
...
char* FileUtility::GetFileContent( const rString& filePath, std::ios_base::openmode openMode ) {
	...
	if( inStream.is_open() ) {
		...
		if( bufferSize > 0 ) {
			fileData = static_cast<char*>( rMalloc( bufferSize ) );
			GetFileContent( filePath, bufferSize, openMode, fileData );
		} else {
...
void FileUtility::GetFileContent( const rString& filePath, size_t readCount, std::ios_base::openmode openMode, char* const outBuffer ) { // TODODB: Make it so we donät need to open the file here, should take a stream as parameter
	...
	inStream.open( filePath.c_str(), openMode );
	if( inStream.is_open() ) {
		inStream.read( outBuffer, readCount );
		inStream.close();
	} else {
		Logger::Log( "GetFileContent was unable to open file (Path = " + filePath + " )", "FileUtility", LogSeverity::WARNING_MSG );
	}
...
rString FileUtility::GetFileContentAsString( const rString& filePath, std::ios_base::openmode openMode ) {
	...
	char* fileContent = static_cast<char*>( rMalloc( fileContentSize + 1) );
	GetFileContent( filePath, fileContentSize, openMode, fileContent );
