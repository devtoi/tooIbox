...
rMap<rString, Config::ConfigEntry*>* Config::CreateNeededMaps( const rString& keyMapPart ) {
	...
	while ( getline( iss, val, '.' ) ) {
		...
		if ( mapIt == toAddTo->end() ) {
			...
			newMap = new rMap<rString, ConfigEntry*>; // TODOJM: Change when not using std::allocator
			ce->Type = Type::Map;
			ce->Value.Map = newMap;
...
rString Config::GetString( const rString& key, const rString& defaultValue, const rString& comment ) {
	...
	if ( ce != nullptr ) {
		...
		if ( !PutEntry( key, cfgEntry ) ) {
			Logger::Log( "Failed to add key: " + key + "with value: " + defaultValue + ", returning default value: " + defaultValue,
...
double Config::GetDouble( const rString& key, double defaultValue, const rString& comment ) {
	...
	if ( ce != nullptr ) {
		...
		if ( !PutEntry( key, cfgEntry ) ) {
			Logger::Log( "Failed to add key: " + key + "with value: " + rToString( defaultValue ) + ", returning default value: " + rToString( defaultValue ),
						 "Config", LogSeverity::WARNING_MSG );
			pDelete( cfgEntry );
		} else {
			m_Dirty = true;
...
bool Config::Parse( const rString& strToParse, rMap<rString, ConfigEntry*>* mapToAddTo ) {
	...
	while ( !iss.eof() ) {
		...
		if ( c == ' ' || c == '\n' || c == '\r' || c == '\t' ) {
			...
			mp = new rMap<rString, ConfigEntry*>; // TODOJM: Change when not using std::allocator
			stage = Stage::Key;
		...
		if ( c == ',' ) {
			...
			if ( type == Type::Int ) {
				...
				try {
					...
					} else { // Contained a "."; hopefully a double.
			...
			} else if ( type == Type::Bool ) { // It is a bool
...
bool Config::ParseArray( const rString& arrayStr, rVector<ConfigEntry*>* vec ) {
	...
	while ( getline( issa, val, ',' ) ) {
		...
		try {
			...
			} else if ( val.find_first_of( "." ) != rString::npos ) { // Possible double?
...
Config::ConfigEntry* Config::AssertFind( const rString& key, bool logNotFoundError ) const {
	...
	if ( p == rString::npos ) {
		...
		} else { // Was not trying to access array
